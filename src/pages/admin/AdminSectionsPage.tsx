...
import { Loader2, Eye, EyeOff, ArrowUp, ArrowDown, Trash2, Copy } from 'lucide-react';
import { fetchPageSections, updatePageSection, deletePageSection, createPageSection } from '@/lib/data';
...
export function AdminSectionsPage() {
  ...
  const toggleVisible = async (section: PageSectionRow) => {
    try {
      await updatePageSection(section.id, { visible: !section.visible });
  ...
  const move = async (index: number, direction: 'up' | 'down') => {
    ...
    try {
      await updatePageSection(a.id, { sort_order: b.sort_order });
      await updatePageSection(b.id, { sort_order: a.sort_order });
      await load();
    } catch {
  ...
  return (
    <div>
      <AdminPageHeader
        title="Блоки и секции"
        description="Управление порядком, видимостью и дублированием блоков главной страницы."
      />
        ...
        {sections.map((section, index) => (
          <div
            ...
            className={`flex items-center gap-3 rounded-2xl p-4 ring-1 ring-ink-200/60 ${
              section.visible ? 'bg-pearl-50' : 'bg-pearl-100/50 opacity-60'
            }`}
          >
            ...
            <div className="flex-1">
              ...
              {section.subtitle && (
                <span className="text-sm text-ink-500">{section.subtitle}</span>
              )}
              <span className="ml-2 font-mono text-xs text-ink-400">{section.section_key}</span>
            </div>
